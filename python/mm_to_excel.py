...
v1.0.5.0
    添加测试用例优先级字段.
v1.0.6.0
    输入0, 代表将根节点下一级中所有包含测试用例的子节点作为起点. 建立多个sheet
"""
...
print("#"*60)
sheet_name = input("请输入测试用例生成的开始节点（该节点将作为工作蒲sheet的表名，下一级将作为【测试主模块】，若不输入任何值则以思维导图的根节点为起点.  输入0则将根节点下一级中所有包含测试用例的子节点作为起点）:\n")
...
def create_testcase_sheet(sheet, top_node):
    '''创建测试用例表格'''
...
#create_testcase_sheet(sheet, top_node)
# 新建工作蒲
wb = Workbook()


# 如果获取到0, 则输出根节点下所有包含测试用例的子节点作为起点, 输出多个sheet工作簿
if sheet_name == "0":
    top_node = root.find("./node[@TEXT]")
    count = 0
    for sub_node in top_node:
        if sub_node.tag != "node":
            continue
        if sub_node.get("TEXT").find("测试用例") != -1:
            if count == 0:
                sheet = wb.active
                sheet.title = sub_node.get("TEXT")
            else:
                sheet = wb.create_sheet(sub_node.get("TEXT"), count)
            create_testcase_sheet(sheet, sub_node);
            count = count + 1
